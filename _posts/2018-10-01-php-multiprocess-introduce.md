...
language: chinese
category: [PHP]
keywords: PHP,multi-process
description: PHP多进程编程实战以及遇到的坑。
---
...
@returns int
int pcntl_fork(void)
成功时，在父进程执行线程内返回产生的子进程PID，在子进程执行线程内返回0，
...
通过调用php创建子进程接口完成一个子进程的创建，pcntl_fork返回值为0证明进入到子进程内，
非0则进入到父进程内部，-1则父进程创建子进程失败。
```
